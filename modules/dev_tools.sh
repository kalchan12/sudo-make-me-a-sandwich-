...
show_dev_tools_menu() {
    _render_menu DEV_TOOLS_LIST "Dev Tools" \
        install_dev_tools check_dev_tools_installations show_main_menu
}

install_dev_tools() { _install_list "Dev Tools" DEV_TOOLS_LIST; }

check_dev_tools_installations() {
    _check_installations DEV_TOOLS_LIST \
        "tmux:tmux" "ripgrep:rg" "bat:bat" "batcat:batcat" \
        "fd:fd" "fdfind:fdfind" "httpie:http" \
        "Neovim:nvim" "Docker:docker" "jq:jq" \
        "fzf:fzf" "btop:btop" "lazygit:lazygit" \
        "zoxide:zoxide" "delta:delta" "tldr:tldr" \
